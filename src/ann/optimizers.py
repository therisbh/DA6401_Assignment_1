...
class SGD:
    ...
    def update(self, layers): self.step(layers)
...
class Momentum:
    ...
    def update(self, layers):
        if not self.vW: self.init_state(layers)
        self.step(layers)
...
class RMSProp:
    def __init__(self, lr, weight_decay=0.0, beta=0.9, eps=1e-8, **kwargs):
        self.lr = lr; self.wd = weight_decay; self.beta = beta; self.eps = eps
        self.sW = []; self.sb = []
    ...
    def update(self, layers):
        if not self.sW: self.init_state(layers)
        self.step(layers)
